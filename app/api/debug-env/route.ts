...
export async function GET() {
  ...
  const allEnvKeys = Object.keys(process.env).filter(key => 
    key.includes('CMS') || key.includes('PASSWORD') || key.includes('cms') || key.includes('password')
  );
  ...
  return NextResponse.json({
    ...
    passwordLastChar: passwordValue !== 'NOT_SET' && passwordLength > 0 ? passwordValue[passwordLength - 1] : null,
